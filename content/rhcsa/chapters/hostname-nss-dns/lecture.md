...
<!-- 稳定 ID、来源和候选状态属于维护层；正式渲染时不应显示。 -->

# 第 19 章　主机名、NSS 与 DNS 名称解析

一台 Linux 主机“叫什么名字”，和一个应用“怎样把名字转换为地址”，是两个相互关联但并不相同的问题。静态主机名保存在本机；`/etc/hosts` 保存本地映射；`/etc/nsswitch.conf` 决定系统解析库按什么来源和顺序寻找答案；NetworkManager 保存 DNS 服务器与搜索域；DNS 服务器分别维护 A、AAAA、PTR 等记录。最后，应用还可能保留自己的缓存或已经建立的连接。

如果把这些层次混在一起，常见误判会立即出现：`hostnamectl` 显示正确便认定 DNS 正常；`dig` 得到答案便认定应用一定能解析；A 记录存在便认定反向 PTR 也存在；当前 `/etc/resolv.conf` 正确便认定重启后一定保持；修改 `/etc/hosts` 后便以为其他主机也会得到相同结果。
...
**[概念]** 主机名（hostname）是本机身份的一部分；DNS 名称是分布式名称空间中的节点名称。二者可以相同，也可以因配置不完整而不同。设置主机名不会自动创建 DNS 记录。

**[概念]** NSS（Name Service Switch）为系统库提供统一的数据源选择机制。`hosts` 只是 NSS 的一个数据库；它可能按当前配置使用 `files`、`dns`、`myhostname`、`resolve` 等来源。

**[概念]** DNS server 决定向谁查询；search domain 决定怎样扩展短名；A、AAAA、PTR 是独立记录类型。正向解析成功并不蕴含反向解析成功。

**[操作语义]** `hostnamectl` 观察和修改主机名；`getent` 通过 NSS 查询系统数据库；`dig` 直接检查 DNS 层；`nmcli` 修改 NetworkManager connection profile 中的持久 DNS 设置。

**[验证原则]** 每次变更至少分开验证：配置真源、当前有效状态、名称查询结果和应用功能。任意一层成功都不能代替下一层。

<section class="topic knowledge" id="RHCSA-19-K01" data-kind="knowledge-topic">

## [知识专题] 一台主机为什么同时存在三种 hostname
...
### ① [知识点] static hostname 是重启后应恢复的持久身份
...
### ② [知识点] transient hostname 表示当前运行期的临时名称
...
### ③ [知识点] pretty hostname 是描述，不是 DNS 标签
...
**[Cheatsheet]** static 看持久身份；transient 看当前临时值；pretty 只作描述；`hostname` 看当前内核名称；`/etc/hostname` 看持久真源；设置主机名不等于发布 DNS。

</section>

<section class="topic knowledge" id="RHCSA-19-K02" data-kind="knowledge-topic">

## [知识专题] 短名、FQDN、绝对名称与搜索域

同一个目标可能以 `node1`、`node1.training.example` 或 `node1.training.example.` 出现。这些字符串不是简单的长短差异：它们决定解析器是否使用搜索域、怎样生成候选查询，以及诊断时是否存在歧义。最稳妥的切入路径是分别测试短名、FQDN 和带末尾点的绝对名称。
...
### ③ [知识点] search domain 只解决名称补全，不决定 DNS 服务器内容

`search training.example` 告诉解析器怎样扩展短名，但不会创建任何记录。要让 `node1` 成功，扩展后的名称仍必须在 `/etc/hosts`、DNS 或其他 NSS 来源中存在。

search 列表按顺序尝试候选域。列表越长，短名失败可能产生越多查询，也更容易因同名资源导致歧义。

### ④ [知识点] `ndots` 会影响多标签名称的尝试顺序
...
**[Cheatsheet]** 短名依赖 search；FQDN 减少歧义；末尾点表示绝对名称；search 不创建记录；应用短名验证优先用 `getent`，DNS 记录验证优先用 `dig`。

</section>

<section class="topic knowledge" id="RHCSA-19-K03" data-kind="knowledge-topic">

## [知识专题] NSS 的 hosts 数据库如何选择解析来源
...
修改这类全局策略风险较高。没有明确任务和证据时，不应为了让一次查询成功而重排整个 `hosts:` 行。

### ④ [知识点] `getent` 是观察 NSS 结果的主要接口
...
**[Cheatsheet]** 先读真实 `hosts:` 行；顺序决定冲突优先级；中括号动作可能提前停止；`getent` 观察 NSS；NSS 成功不等于业务终态。

</section>

<section class="topic operation" id="RHCSA-19-O01" data-kind="operation-topic">

## [操作专题] 查询和设置主机名并区分当前与持久状态
...
若题目明确要求重启后保持，而当前没有 live VM，讲义只能把真实重启列入后续验证，不得声称已经完成。
...
**[Cheatsheet]** A 查 IPv4；AAAA 查 IPv6；`dig -x` 查 PTR；正反向独立；状态码要和 ANSWER 一起看。

</section>

<section class="topic operation" id="RHCSA-19-O03" data-kind="operation-topic">

## [操作专题] 通过 NetworkManager 配置 DNS server 与 search domain
...
### ② [诊断] `hostnamectl` 正确而 `hostname -f` 异常
...
### ⑤ [诊断] 多个 search domain 导致短名命中错误目标
...
**最小修复：** 删除或修正唯一的过期条目；保持合理 NSS 策略。不要通过全局重排 `hosts:` 行来绕开坏数据。

### ② [诊断] 指定服务器 `dig` 成功而普通 `dig` 失败
...
**[Cheatsheet]** `dig` 对、`getent` 错先查 hosts/NSS；指定服务器对、默认路径错先查 resolver；`getent` 对、应用错查应用缓存和连接；名称都对则离开 DNS 层。

</section>

<section class="topic classic-task" id="RHCSA-19-T01" data-kind="classic-task">

## [经典任务] 配置静态主机名、本地映射和 DNS 搜索域
...
<section class="topic classic-task" id="RHCSA-19-T02" data-kind="classic-task">

## [经典任务] 诊断 `dig` 正确但应用得到错误地址
...
<section class="topic answer" id="RHCSA-19-A01" data-kind="reference-answer">

## [参考解答] 经典任务一：配置主机名、本地映射和搜索域

参考解答不是唯一命令序列，而是一条可审计的状态迁移链。以下命令未在本会话连接 RHEL 9 VM 执行；必须在真实环境按接口名、profile 状态和题目给定记录调整。
...
本章只能确认命令、配置路径和逻辑闭环已经静态核对。真实环境仍需重启系统，重新检查 hostname 与 `exam-net` 自动激活状态，并重复 `getent`、`dig` 和应用功能测试。

</section>

<div class="page-break"></div>

<section class="topic answer" id="RHCSA-19-A02" data-kind="reference-answer">

## [参考解答] 经典任务二：从 `dig`/`getent` 差异定位最小修复
