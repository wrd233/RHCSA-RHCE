...
from common import iter_chapters, load_yaml
...
def main() -> int:
    ...
    expected_notes = 0
    expected_cards = 0
    for chapter in iter_chapters(["common", "rhcsa"]):
        for note in load_yaml(chapter["path"] / "anki.yml")["notes"]:
            if note.get("disabled"):
                continue
            expected_notes += 1
            expected_cards += len(set(re.findall(r"\{\{c(\d+)::", note.get("text", "")))) if note["type"] == "cloze" else 1
    report["expected_notes"] = expected_notes
    report["expected_cards"] = expected_cards
    if report["notes"] != expected_notes or report["cards"] != expected_cards or duplicates or not report["templates_hide_source"]:
        report["errors"].append("readback invariant failed")
