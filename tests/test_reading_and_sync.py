...
def test_semantic_concept_and_operation_markup():
    source="**[概念]** 词是单位。用于形成参数。\n\n**[操作语义]** `type` 查看名称。"
    html=lecture_markdown_to_html(source)
    assert "concept-term" not in html and "operation-quickref" not in html
    explicit='<div class="concept-block"><span class="concept-label">概念</span><p><strong>词</strong>是作者解释。</p></div>'
    rendered=lecture_markdown_to_html(explicit)
    assert 'class="concept-term"' in rendered and 'class="concept-explanation"' in rendered
    assert "作者解释" in rendered
